...
[fc_accuracy(generalised).sh](./fc_accuracy(generalised).sh) is the evolution [fc_accuracy(single_day).sh](./fc_accuracy(single_day).sh); indeed, its same code structure was implemented to calculate the accuracy of all the predictions reported in the file [rx_poc.log](./rx_poc.log).  
